...
const Main = ({ userinfo }) => {
  const { reference, logout } = useReference();
  const [activePage, setActivePage] = useState("total");

  if (!reference) return <LogInPage />;

  const filteredUser = userinfo.filter((item) => item.reference === reference);

  return (
    <div className="flex h-screen w-full bg-gray-100">
      {/* Sidebar */}
      <div className="hidden md:flex flex-col w-60 bg-gray-900 text-white p-6">
        <h2 className="text-2xl font-bold mb-8">{reference} PANEL</h2>

        <nav className="flex flex-col gap-4">
          <button
            onClick={() => setActivePage("total")}
            className={`px-3 py-2 rounded-lg ${
              activePage === "total" ? "bg-gray-700" : "hover:bg-gray-800"
            }`}
          >
            Total Connections
          </button>
          ...
          <button
            onClick={() => setActivePage("paid")}
            className={`px-3 py-2 rounded-lg ${
              activePage === "paid" ? "bg-gray-700" : "hover:bg-gray-800"
            }`}
          >
            Paid
          </button>
          ...
          <button
            onClick={() => setActivePage("notpaid")}
            className={`px-3 py-2 rounded-lg ${
              activePage === "notpaid" ? "bg-gray-700" : "hover:bg-gray-800"
            }`}
          >
            Not Paid
          </button>
      ...
      <div className="flex-1 p-4 overflow-auto bg-gray-200">
        {activePage === "total" && <TotalConnection userInfo={filteredUser} />}

        {activePage === "paid" && <Paid userInfo={filteredUser} />}

        {activePage === "notpaid" && <NotPaid userInfo={filteredUser} />}
      </div>
    </div>
  );
};
